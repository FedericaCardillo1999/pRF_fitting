...
# psc_avg_ts_full=np.load(f'{MAIN_PATH}/pRFM/{subject}/ses-02/{denoising}/{subject}_ses-02_task-RET_hemi-LR_desc-avg_bold_{depth}.npy').T[:,8:]                  
#psc_avg_ts_full=np.load(f'{MAIN_PATH}/pRFM/{subject}/ses-01/{denoising}/{subject}_ses-01_task-{task}_hemi-LR_desc-avg_bold_{depth}.npy').T[:,8:]                  
# print(psc_avg_ts_full.shape)
	
# Build paths
base_path_01 = f'{MAIN_PATH}/pRFM/{subject}/ses-01/{denoising}/{subject}_ses-01_task-{task}_hemi-LR_desc-avg_bold_{depth}.npy'
base_path_02 = f'{MAIN_PATH}/pRFM/{subject}/ses-02/{denoising}/{subject}_ses-02_task-{task}_hemi-LR_desc-avg_bold_{depth}.npy'

# Try loading from ses-01, fall back to ses-02
if os.path.exists(base_path_01):
    file_path = base_path_01
elif os.path.exists(base_path_02):
    file_path = base_path_02
else:
    raise FileNotFoundError("Neither ses-01 nor ses-02 data file was found.")

# Load and process
psc_avg_ts_full = np.load(file_path).T[:, 8:]	
	
#if psc_avg_ts_full.shape[1]>225:
#    psc_avg_ts_full=psc_avg_ts_full[:,:]
...
# The original script: 
sizes = np.random.uniform(0.01, max_eccentricity, 30) 
...
# Define the bounds to cover the entire range of pRF sizes, from 0.01 to max eccentricity
gauss_iter_bounds = [
    (-1.1 * max_eccentricity, 1.1 * max_eccentricity),  # x bound
    (-1.1 * max_eccentricity, 1.1 * max_eccentricity),  # y bound
    (0.01, 1 * max_eccentricity),                      # PRF size bounds (full range from 0.01 to max eccentricity)
