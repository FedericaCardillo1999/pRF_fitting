function main_spmmoco_4runs(project, subject)
...
myfilespath =['/scratch/hb-EGRET-AAA/projects/EGRET+/derivatives/spm/'];
cd(myfilespath)
subjects=dir;
spm_jobman('initcfg');
...
%copy func files
cd ([myfilespath 'sub-' subject '/ses-01/no_moco']);      %%%%%% mudar ses 
niigzFiles=dir('*nii.gz')
for f=1:numel(niigzFiles)
    niigzFile=niigzFiles(f).name
    gunzip(niigzFile,[myfilespath 'sub-' subject '/ses-01/func'])   %%%%% mudar ses 
end
cd([myfilespath 'sub-' subject '/ses-01/func']);   %%%%% mudar ses 
